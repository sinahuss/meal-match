...
def test_post_recipe_valid(client):
    ...
    with patch('app.api.recipes_api.create_recipe', return_value='abc123') as mock_create:
...
def test_post_recipe_invalid(client):
    ...
    with patch('app.api.recipes_api.create_recipe', side_effect=ValueError("Recipe 'name' is required.")):
        response = client.post('/recipes', json=data)
...
def test_get_recipes_empty(client):
    with patch('app.api.recipes_api.get_all_recipes', return_value=[]) as mock_get_all:
...
def test_get_recipes_nonempty(client):
    ...
    with patch('app.api.recipes_api.get_all_recipes', return_value=recipes):
        response = client.get('/recipes')
        assert response.status_code == 200
        assert response.get_json() == recipes 

def test_delete_recipe_success(client):
    with patch('app.api.recipes_api.delete_recipe', return_value=1) as mock_delete:
...
def test_delete_recipe_not_found(client):
    with patch('app.api.recipes_api.delete_recipe', side_effect=ValueError('Recipe not found')):
        response = client.delete('/recipes/abc123')
        assert response.status_code == 404
        assert response.get_json()['error'] == 'Recipe not found'

def test_delete_recipe_invalid_id(client):
    with patch('app.api.recipes_api.delete_recipe', side_effect=Exception('Invalid recipe ID')):
...
def test_get_recipe_by_id_success(client):
    ...
    with patch('app.api.recipes_api.get_recipe_by_id', return_value=recipe) as mock_get:
...
def test_get_recipe_by_id_not_found(client):
    with patch('app.api.recipes_api.get_recipe_by_id', side_effect=ValueError('Recipe not found')):
        response = client.get('/recipes/abc123')
        assert response.status_code == 404
        assert response.get_json()['error'] == 'Recipe not found'

def test_get_recipe_by_id_invalid_id(client):
    with patch('app.api.recipes_api.get_recipe_by_id', side_effect=Exception('Invalid recipe ID')):
